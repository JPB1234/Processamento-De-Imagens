...
def thresholding_segmentation(img_cv, canvas):
    ...
    edited_img = cv2.cvtColor(binary_img, cv2.COLOR_GRAY2BGR)
    display_image(edited_img, canvas, original=False)
...
def erosion(img_cv, canvas, kernel_size = 5):
    ...
    edited_img = cv2.cvtColor(output_img, cv2.COLOR_GRAY2BGR)
    display_image(edited_img, canvas, original=False)
...
def open(img_cv, canvas, kernel_size=5):
    ...
    edited_img = cv2.cvtColor(dilated_img, cv2.COLOR_GRAY2BGR)
    display_image(edited_img, canvas, original=False) 
...
def close(img_cv, canvas, kernel_size=5):
    ...
    edited_img = cv2.cvtColor(erosion_img, cv2.COLOR_GRAY2BGR)
    display_image(edited_img, canvas, original=False) 
...
def low_pass(img_cv, canvas):
    if img_cv is None:
        return
    edited_img = cv2.GaussianBlur(img_cv, (15, 15), 0)
    display_image(edited_img, canvas, original=False)
...
def low_pass_gaussian(img_cv, canvas, sigma=3.0):
    ...
    edited_img = GuassianBlur(img_cv, sigma)
    if not isinstance(edited_img, np.ndarray):
...
def low_pass_media(img_cv, canvas):
    ...
    edited_img = low_pass_mean_filter(img_cv)
    if not isinstance(edited_img, np.ndarray):
        raise TypeError("not nparray")
...
def high_pass(img_cv, canvas):
    ...
    edited_img = cv2.Laplacian(gray, cv2.CV_64F)
    edited_img = cv2.convertScaleAbs(edited_img)
...
def high_pass_laplacian(img_cv, canvas, kernel_value):
    ...
    edited_img = cv2.filter2D(gray, -1, kernel)
    edited_img = cv2.cvtColor(edited_img, cv2.COLOR_GRAY2BGR)
    display_image(edited_img, canvas, original=False)
...
def high_pass_sobel(img_cv, canvas):
    ...
    edited_img = sobel_filter_manual(img_cv)
    display_image(edited_img, canvas, original=False)
...
def GuassianBlur(img: np.ndarray, sigma: Union[float, int], filter_shape: Union[List, Tuple, None] = None):
    ...
    edited_img = cv2.filter2D(img, -1, gaussian_filter)
    return edited_img.astype(np.uint8)

def manual_convolution(image, kernel):
    image_height, image_width = image.shape
    kernel_height, kernel_width = kernel.shape
    edited_img = np.zeros_like(image)
...
def sobel_filter_manual(img, direction='x'):
    ...
    edited_img = cv2.filter2D(gray, cv2.CV_64F, kernel)

    return cv2.convertScaleAbs(edited_img)

def aplying_erosion(img_cv, kernel_size = 5):
    pad_size = kernel_size // 2
    padded_img = np.pad(img_cv, pad_size, mode='constant', constant_values=255)
    edited_img = np.zeros_like(img_cv)
    ...
    for i in range(pad_size, padded_img.shape[0] - pad_size):
        for j in range(pad_size, padded_img.shape[1] - pad_size):
            ...
            edited_img[i - pad_size, j - pad_size] = np.min(region)
    return edited_img
...
def aplying_dilatation(img_cv, kernel_size = 5):
    ...
    for i in range(pad_size, padded_img.shape[0] - pad_size):
        for j in range(pad_size, padded_img.shape[1] - pad_size):
            region = padded_img[i - pad_size:i + pad_size + 1, j - pad_size:j + pad_size + 1]
            edited_img[i - pad_size, j - pad_size] = np.max(region)
    return edited_img
...
def aplying_otsu(gray_img):
    ...
    edited_img = np.zeros_like(gray_img)
    edited_img[gray_img >= threshold_value] = 255
    return edited_img
